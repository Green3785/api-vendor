...
class MockGraphQLClientFactory {
    configService;
    socketConfig;
    constructor(configService, socketConfig) {
...
class ClientConsumerService {
    ...
    client = null;
    wsClient = null;
    constructor(factory, apiKeyProvider, options = {}) {
    ...
    describe('configuration scenarios', () => {
        it('should handle Unix socket configuration', async () => {
            vi.mocked(configService.get).mockImplementation((key, defaultValue) => {
                if (key === 'PORT')
                    return '/var/run/unraid-api.sock';
                return defaultValue;
            });
    ...
    describe('integration scenarios', () => {
        it('should handle production scenario with Unix socket and subscriptions', async () => {
            vi.mocked(configService.get).mockImplementation((key, defaultValue) => {
                if (key === 'PORT')
                    return '/var/run/unraid-api.sock';
                if (key === 'store.emhttp.nginx.httpPort')
                    return '80';
                return defaultValue;
            });
            const socketConfig = new SocketConfigService(configService);
...
//# sourceMappingURL=internal-graphql-client-usage.spec.js.map