 ...
 */
export function parseActionToAuthAction(action, defaultPossession = 'ANY') {
    if (!action)
        return null;
    // First check if it's already a valid AuthAction value
    if (Object.values(AuthAction).includes(action)) {
        return action;
    }
    ...
    if (Object.values(AuthAction).includes(normalized)) {
        return normalized;
    }
    ...
    if (parts.length === 1 && simpleVerbs.includes(verb)) {
        const withPossession = `${verb}_${defaultPossession}`;
 ...
 */
export function parseResourceToEnum(resourceStr) {
    const normalized = resourceStr.trim().toUpperCase();
    // Direct enum lookup
    const directMatch = Resource[normalized];
    if (directMatch) {
        return directMatch;
    }
    return null;
 ...
 */
export function parseRoleToEnum(roleStr) {
    const normalized = roleStr.trim().toUpperCase();
    const role = Role[normalized];
    return role || null;
}
/**
 ...
 */
export function convertScopesToPermissions(scopes) {
    const permissions = [];
    const roles = [];
 ...
 */
export function convertPermissionsToScopes(permissions, roles = []) {
    const scopes = [];
    // Add role scopes
 ...
 */
export function roleToScope(role) {
 ...
 */
export function permissionToScope(resource, action) {
 ...
 */
export function normalizeLegacyAction(action) {
 ...
 */
export function normalizeLegacyActions(actions) {
    return actions
 ...
 */
export function expandWildcardAction() {
 ...
 */
export function reconcileWildcardPermissions(permissionsWithSets) {
    if (permissionsWithSets.has('*')) {
        const wildcardActions = permissionsWithSets.get('*');
        permissionsWithSets.delete('*');
 ...
 */
export function mergePermissionsIntoMap(targetMap, sourceMap) {
 ...
 */
export function convertPermissionSetsToArrays(permissionsWithSets) {
    const result = new Map();
    for (const [resource, actionsSet] of permissionsWithSets) {
        if (resource !== '*') {
            result.set(resource, Array.from(actionsSet));
        }
    }
    return result;
}
